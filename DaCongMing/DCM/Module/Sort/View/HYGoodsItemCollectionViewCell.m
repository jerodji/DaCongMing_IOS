...
- (void)setGoodsModel:(HYGoodsItemModel *)goodsModel{
    ...
    [_imgView sd_setImageWithURL:[NSURL URLWithString:goodsModel.item_title_image]];
    ...
    NSMutableAttributedString *origin = [NSMutableAttributedString attachmentStringWithContent:label contentMode:UIViewContentModeCenter attachmentSize:CGSizeMake(label.width, label.height) alignToFont:KFitFont(13) alignment:YYTextVerticalAlignmentCenter];
    NSMutableAttributedString *title = [[NSMutableAttributedString alloc] initWithString:goodsModel.item_name attributes:@{NSFontAttributeName : KFitFont(12),NSForegroundColorAttributeName : KAPP_272727_COLOR}];
    
    [attStr appendAttributedString:origin];
    [attStr appendString:@" "];
...
- (UIImageView *)imgView{
    ...
    if (!_imgView) {
        ...
        _imgView.layer.cornerRadius = 5;//self.whiteBgView.layer.cornerRadius;
    }
    return _imgView;
}
...
- (YYLabel *)titleLabel{
    ...
    if (!_titleLabel) {
        ...
        _titleLabel.font = [UIFont systemFontOfSize:10];//KFitBoldFont(10);
